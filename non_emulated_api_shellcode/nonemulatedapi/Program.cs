...
namespace ConsoleApp1
{
	class Program
	{
		...
		[DllImport("kernel32.dll")]
		static extern IntPtr GetCurrentProcess();   //execute things is avaible current process 
		static void Main(string[] args)
		{
			// IntPtr hProcess = OpenProcess(0x001F0FFF, false, 7212); ( now we dont need this as we are checking thins fromcurrent processapi)
			IntPtr mem = VirtualAllocExNuma(GetCurrentProcess(), IntPtr.Zero, 0x1000, 0x3000, 0x4,0); 
			
			// in case of single cpu we need to mention it to 0 as it will not found the phsical memory residing in the machine if api is not emulated
			 // generally we get physical address in multi process so api emulates it so we will exit it without harming it  
			if (mem == null)
			{
				return;
			}
